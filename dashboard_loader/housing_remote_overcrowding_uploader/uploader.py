...
from housing_remote_overcrowding_uploader.models import *
    ...
    try:
        ...
        messages.extend(
                load_state_grid(wb, "Data",
                                "Housing", "Remote Overcrowding",
                                None, HousingRemoteOvercrowdingData,
                                {}, {"percentage": "Proportion of indigenous persons in remote areas (%)",},
                                multi_year=True,
                                verbosity=verbosity)
        )
        desc = load_benchmark_description(wb, "Description", indicator=True)
        messages.extend(update_stats(desc, None,
                                "remote_overcrowding-housing-hero", "remote_overcrowding-housing-hero",  
                                None, None,
                                "housing_remote_overcrowding", "housing_remote_overcrowding",  
                                None, None,
                                verbosity))
        qry = HousingRemoteOvercrowdingData.objects.filter(state=AUS).order_by("year")
        ref = qry.first()
        latest = qry.last()
        ...
        g = get_graph("housing_remote_overcrowding", "housing_remote_overcrowding",  
                    "housing_remote_overcrowding_detail_graph", )
        clear_graph_data(g, clusters=True)
        sort_order = 5
        for datum in qry:
            cluster = add_graph_dyncluster(g, datum.year_display(), sort_order, datum.year_display())
            gd = add_graph_data(g, "overcrowding", datum.percentage, cluster=cluster)
        messages.extend(
                populate_raw_data("housing_remote_overcrowding", "housing_remote_overcrowding",  
                                "housing_remote_overcrowding", HousingRemoteOvercrowdingData,
                                {
                                    "percentage": "percentage_overcrowded",
                                })
                )
        messages.extend(
                populate_crosstab_raw_data("housing_remote_overcrowding", "housing_remote_overcrowding",  
                                "data_table", HousingRemoteOvercrowdingData,
                                {
