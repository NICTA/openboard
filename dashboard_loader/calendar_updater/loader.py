...
def update_data(loader, verbosity=0):
    ...
    existing_today_items = get_statistic("events", "day", "today").get_data()
    correct_today_items = get_statistic("events", "day", "calendar").get_data().filter(datekey=today)
    ...
    for correct_item in correct_today_items:
        ...
        for item in existing_today_items:
            if item.strval == correct_item.strval and item.url == correct_item.url and item.sort_order == correct_item.sort_order:
                found = True
        if not found:
            add_statistic_list_item("events", "day", "today", correct_item.value(), correct_item.sort_order, 
